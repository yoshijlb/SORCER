...
class StartAll {
    ...
    ServiceDescriptor[] getServiceDescriptors() {
        ...
        def descriptors = []
        def configArg = ["${configPath}/ssu-prv.config"]
        def codebase = "${relativeRepoPath}/ssu-${sorcerVersion}-dl.jar ${relativeRepoPath}/ssu-${sorcerVersion}-ui.jar sorcer-dl-${sorcerVersion}.jar jsk-dl-${riverVersion}.jar"

        descriptors << new SorcerServiceDescriptor(codebase,
                                                   policy,
                                                   "${buildLibPath}/ssu-${sorcerVersion}-prv.jar${File.pathSeparator}${buildLibPath}/ssu-${sorcerVersion}-ui.jar",
